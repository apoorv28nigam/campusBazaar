...
export default function Checkout() {
  ...
  const handlePayment = async () => {
    setPayLoading(true);
    try {
      const res = await paymentsAPI.checkout({ itemId: item._id, type: 'buy' });
      // Redirect to Stripe or external payment URL
      if (res.data.url) {
  ...
  const platformFee = Math.round(item.price * 0.02); // example 2% fee
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 400px', gap: 40, alignItems: 'start' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start', padding: 20, borderRadius: 16, background: 'var(--primary-light)', border: '1px solid rgba(107, 79, 58, 0.1)' }}>
                  ...
                  <p style={{ color: 'var(--primary)', fontSize: 14, fontWeight: 600, lineHeight: 1.5 }}>
                    Your payment is held securely in escrow. Funds are only released to the seller once you confirm receipt of the item.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="card" style={{ padding: 32, position: 'sticky', top: 120, border: '1px solid var(--border)', boxShadow: '0 10px 30px rgba(0,0,0,0.05)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20, marginBottom: 32 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--text-muted)', fontSize: 16, fontWeight: 600 }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>Service Processing <Info size={16} /></span>
                <span style={{ color: 'var(--text)', fontWeight: 700 }}>₹{platformFee.toLocaleString('en-IN')}</span>
              </div>
            ...
            <button onClick={handlePayment} disabled={payLoading} className="btn-primary" style={{ width: '100%', justifyContent: 'center', padding: '18px', fontSize: 18, fontWeight: 800, marginBottom: 20, boxShadow: '0 8px 20px rgba(107, 79, 58, 0.2)' }}>
              {payLoading ? 'Processing...' : (
                <><ShoppingBag size={22} /> Pay & Secure Item</>
              )}
            </button>

            <div style={{ display: 'flex', gap: 8, alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontSize: 13, fontWeight: 600 }}>
              <CreditCard size={16} /> Fully Secured via Stripe
            </div>
